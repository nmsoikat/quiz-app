...
class CreateQuiz extends react.Component {
  ...
  state = {
    ...
    quizTemp: {
      ...
      description:
        "This is dummy description Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod delectus repudiandae repellat consequatur aliquam explicabo accusamus. Aliquid, sapiente! Exercitationem asperiores cupiditate numquam dolores explicabo corporis porro ipsam mollitia reiciendis libero?",
    },
    questions: [
      {
        _id: uuidv4(),
  ...
  saveQuestion = () => {
    let completeQuestion = {
      ...
      author: {
        ...
        password: "12345",
        picture: "/quiz-app/upload/male-profile-avatar.jpg",
        bio:
      ...
      <div className="create-quiz-wrap">
        <div className="container">
          <div className="quiz-info box-style">
            <div className="quiz-info__title">
              {this.state.title ? (
                ...
                <h2 onClick={this.clickHandler.bind(this)} data-name="title">
                  {title}
                </h2>
              )}
            </div>
              ...
              <div className="options-wrap">
                {this.state.optionCount.map((item) => (
                  <div className="options" key={item._id}>
                    ...
                    />
                    <span
                      className={classNames("options__del-icon", {
                        "d-none": this.state.optionCount.length < 3,
                      })}
                      onClick={() => {
                        this.deleteInputBox(item._id);
                      }}
                    >
                ...
                {this.state.optionCount.length >= 4 ? (
                  ""
                ) : (
                  <span
                    className="options__add mb-5"
                    onClick={this.addInputBox.bind(this)}
                  >
          ...
          <div className="complete-btn my-3 text-right">
            <button onClick={this.saveQuestion} className="btn btn-warning">
              Save All
            </button>
          </div>
        </div>
